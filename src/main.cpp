...
// Copyright (c)	  2015 The SJWcoin developers
...
#if defined(NDEBUG)
# error "SJW cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SJW Signed Message:\n";
...
//TODO: this isn't identical to sjwcoin reference client.
...
// sjwcoin difficulty
static const int64_t nTargetTimespan = 36000;
static const int64_t nTargetTimespanNEW = 3600;
static const int64_t nTargetSpacing = 60;
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
// end sjwcoin difficulty
...
unsigned int ComputeMinWork(unsigned int nBase, int64_t nTime)
{
    ...
    while (nTime > 0 && bnResult < bnLimit)
    {
        if(chainActive.Height()+1<nDiffChangeTarget){
            ...
            bnResult *= 4; //bnResult = (bnResult * 110) / 100;
            // ... in best-case exactly 4-times-normal target time
            nTime -= nTargetTimespan*4; //nTime -= nTargetTimespanNEW*4;
        }
    }
    if (bnResult > bnLimit)
        bnResult = bnLimit;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SJWcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in SJWcoin
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if (!fIsInitialDownload)
    {
		...
		Mpindex = pindex->nVersion;
        if (nUpgraded > 0)
            LogPrintf("SetBestChain: %d of last 100 blocks above version %d\n We have version %d\n", nUpgraded, (int)CBlock::CURRENT_VERSION, (int)Mpindex);
        if (nUpgraded > 100/2)
...
{
    // SJWcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    // Don't know what it is, just say we already got one <-- topkek
    return true;
}
