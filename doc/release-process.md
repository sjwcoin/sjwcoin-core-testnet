Release Process //SJWCoin has no procedure for this yet
====================
...
##perform gitian builds
 ...
 Fetch and build inputs: (first time, or when dependency versions change)
	...
	cd ..
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/boost-linux.yml
	mv build/out/boost-*.zip inputs/
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/deps-linux.yml
	mv build/out/sjwcoin-deps-*.zip inputs/
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/qt-linux.yml
	mv build/out/qt-*.tar.gz inputs/
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/boost-win.yml
	mv build/out/boost-*.zip inputs/
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/deps-win.yml
	mv build/out/sjwcoin-deps-*.zip inputs/
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/qt-win.yml
	mv build/out/qt-*.zip inputs/
	./bin/gbuild ../sjwcoin-core/contrib/gitian-descriptors/protobuf-win.yml
 ...
 Build dogecoind and dogecoin-qt on Linux32, Linux64, and Win32:
	...
	./bin/gbuild --commit sjwcoin=v${VERSION} ../sjwcoin-core/contrib/gitian-descriptors/gitian-linux.yml
	./bin/gsign --signer $SIGNER --release ${VERSION} --destination ../gitian.sigs/ ../sjwcoin-core/contrib/gitian-descriptors/gitian-linux.yml
	pushd build/out
	zip -r sjwcoin-${VERSION}-linux-gitian.zip *
	mv sjwcoin-${VERSION}-linux-gitian.zip ../../../
	popd
	./bin/gbuild --commit sjwcoin=v${VERSION} ../sjwcoin-core/contrib/gitian-descriptors/gitian-win.yml
	./bin/gsign --signer $SIGNER --release ${VERSION}-win --destination ../gitian.sigs/ ../sjwcoin-core/contrib/gitian-descriptors/gitian-win.yml
	pushd build/out
	zip -r sjwcoin-${VERSION}-win-gitian.zip *
	mv sjwcoin-${VERSION}-win-gitian.zip ../../../
	popd
	popd
...
**Linux .tar.gz:**
	...
	unzip sjwcoin-${VERSION}-linux-gitian.zip -d sjwcoin-${VERSION}-linux
	tar czvf sjwcoin-${VERSION}-linux.tar.gz sjwcoin-${VERSION}-linux
	rm -rf sjwcoin-${VERSION}-linux

**Windows .zip and setup.exe:**

	unzip sjwcoin-${VERSION}-win-gitian.zip -d sjwcoin-${VERSION}-win
	mv sjwcoin-${VERSION}-win/sjwcoin-*-setup.exe .
	zip -r sjwcoin-${VERSION}-win.zip sjwcoin-${VERSION}-win
	rm -rf sjwcoin-${VERSION}-win

**Perform Mac build:**

// SJW isn't building mac images yet...
